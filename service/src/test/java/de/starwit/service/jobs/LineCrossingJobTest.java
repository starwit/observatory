...
public class LineCrossingJobTest {
    ...
    public void testLineCrossingSynthetic() throws InterruptedException {
        ...
        // No point on trajectory should be ON the counting line (b/c direction is then
        // ambiguous)
        ...
        ArgumentCaptor<LineCrossingObservation> observationCaptor = ArgumentCaptor
                .forClass(LineCrossingObservation.class);
    ...
    @Test
    public void testLineCrossingDump() throws IOException {
        ObservationJobEntity jobEntity = prepareJobEntity(Arrays.asList(
        ...
        // This list was manually validated, i.e. it is correct (!) given the job
        // coordinates and dumpfile above
    ...
    static List<SaeMessageDto> createLinearTrajectory(Point2D start, Point2D end, int numSteps, Duration stepInterval) {
        ...
        for (int i = 0; i < numSteps; i++) {
            ...
            trajectory.add(Helper.createSaeMsg(startTime.plus(stepInterval.multipliedBy(i + 1)),
                    new Point2D.Double(currentX, currentY), "obj1"));
        }
